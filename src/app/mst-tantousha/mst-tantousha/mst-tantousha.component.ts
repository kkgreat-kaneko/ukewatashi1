...
export class MstTantoushaComponent implements OnInit {
  ...
  */
  ...
  ngOnInit() {
    ...
    /*
    *  checkBoxのselected契機で担当者一覧の選択データを保持する。tantoushaServiceに選択されたselectItemを登録
    *  tantoushaServiceが編集・削除のボタンから利用される
    *  選択、選択解除によってデータ保持リセット行う
    *  一覧は単一選択のみ可、TantoushaServiceで保持している選択データ単一用、解除する処理
    */
